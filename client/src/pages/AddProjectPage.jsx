...
export default function AddProjectPage({ onAdd, onCancel }) {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'var(--bg)', zIndex: 500, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
      ...
      <button
        onClick={onCancel}
        style={{ position: 'absolute', top: 20, left: 20, zIndex: 1, display: 'flex', alignItems: 'center', gap: 8, background: 'transparent', border: '1px solid var(--border)', borderRadius: 8, padding: '8px 14px', color: 'var(--textMuted)', fontFamily: "'TW Cen MT', 'Century Gothic'", fontSize: 14, cursor: 'pointer', transition: 'all 0.2s ease' }}
      ...
      <div style={{ position: 'relative', zIndex: 1, width: '100%', maxWidth: 560, background: 'var(--surface)', backdropFilter: 'blur(4px)', WebkitBackdropFilter: 'blur(4px)', border: '1px solid var(--border)', borderRadius: 16, boxShadow: '0 16px 48px rgba(0,0,0,0.3)', overflow: 'hidden' }}>
        ...
        <div style={{ padding: '28px 32px 0' }}>
          ...
          <p style={{ color: 'var(--textMuted)', fontFamily: "'TW Cen MT', 'Century Gothic'", fontSize: 14, marginBottom: 20 }}>
        ...
        <form onSubmit={handleSubmit} style={{ padding: '24px 32px 28px' }}>
          ...
          {error && (
            <div style={{ marginBottom: 16, padding: '10px 14px', background: 'var(--redTint)', border: '1px solid #EF444430', borderRadius: 8, color: 'var(--red)', fontSize: 13, fontFamily: "'TW Cen MT', 'Century Gothic'" }}>
              {error}
            </div>
          )}
          ...
          <button
            type="submit"
            disabled={loading}
            style={{ width: '100%', background: 'var(--accent)', color: '#fff', borderRadius: 8, padding: '11px', fontFamily: "'TW Cen MT', 'Century Gothic'", fontWeight: 600, fontSize: 15, cursor: loading ? 'not-allowed' : 'pointer', border: 'none', opacity: loading ? 0.7 : 1, transition: 'all 0.2s ease' }}
          >
      ...
      <div style={{ marginBottom: 16 }}>
        ...
        <div style={{ marginTop: 6, display: 'flex', justifyContent: 'flex-end' }}>
          <button
            ...
            style={{ background: 'transparent', border: '1px solid var(--accent)', borderRadius: 6, padding: '5px 14px', color: 'var(--accent)', fontFamily: "'TW Cen MT', 'Century Gothic'", fontSize: 13, cursor: testLoading || !jql.trim() ? 'not-allowed' : 'pointer', opacity: !jql.trim() ? 0.5 : 1, transition: 'all 0.2s ease' }}
          >
          ...
          {builtJql && (
            <div style={{ marginTop: 4, display: 'flex', justifyContent: 'flex-end' }}>
              <button
                ...
                style={{ background: 'transparent', border: '1px solid var(--accent)', borderRadius: 6, padding: '5px 14px', color: 'var(--accent)', fontFamily: "'TW Cen MT', 'Century Gothic'", fontSize: 13, cursor: testLoading ? 'not-allowed' : 'pointer', transition: 'all 0.2s ease' }}
              >
                {testLoading ? 'Testiram...' : '▶ Test upita'}
              </button>
            </div>
...
function TestResult({ result, error }) {
  if (error) {
    return (
      <div style={{ marginTop: 8, padding: '8px 12px', background: 'var(--redTint)', border: '1px solid #EF444430', borderRadius: 6, color: 'var(--red)', fontSize: 12, fontFamily: "'TW Cen MT', 'Century Gothic'" }}>
        {error}
      </div>
    )
  }
    ...
    <div style={{ marginTop: 8, padding: '10px 12px', background: 'var(--greenTint)', border: '1px solid rgba(34,197,94,0.25)', borderRadius: 6 }}>
      ...
      {result.preview.map(p => (
        <div key={p.key} style={{ display: 'flex', gap: 8, alignItems: 'baseline', marginBottom: 3 }}>
          ...
          <span style={{ fontFamily: "'TW Cen MT', 'Century Gothic'", fontSize: 12, color: 'var(--textMuted)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{p.summary}</span>
